...
function waitForInstance(inst) {
  let self = inst === 'db' ? db : (inst === 'wlt' ? wlt : con);
  return new Promise(async function(resolve, reject) {
    try {
      while(typeof self === 'undefined') {
        await timeout(2);
        self = inst === 'db' ? db : (inst === 'wlt' ? wlt : con);
      }
      resolve();
    } catch (e) {
      reject(e);
...
export async function getInstance(inst) {
  ...
  return inst === 'db' ? db : (inst === 'wlt' ? wlt : con);
}
...
export async function callGetHashForTransfer(props) {
  await waitForInstance('wlt');
  return callResolver(wlt.getHashForTransferAsync, [props.receiver, props.token, props.amount, props.msg, props.nonce]);
}

export async function callGetTxNonce(crtId) {
  await waitForInstance('db');
  return callResolver(db.getCrtTxNonceAsync, [crtId]);
}
